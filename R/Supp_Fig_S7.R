## DEgenes for BRCA - histological types
...
### Phi matrix#
#f <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/results_GTM/TCGA/BRCA/DEgenes_type_sparse_75p/1topic/input_phi_normalized.csv",header=FALSE)
f <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/results_gtm-decon/TCGA/BRCA/DEgenes_type_sparse_75p//1topic/trainData_JCVB0_nmar_K10_iter9_phi_normalized.csv",header=FALSE)
genes <-read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/results_gtm-decon/TCGA/BRCA/DEgenes_type_sparse_75p/1topic/genes.txt",header=FALSE)
...
df_diff <- data.frame("Genes" = genes,
                      "D_Basal" = celltype1,
                      "D_Basal_myoepithelial" = celltype2,
                      "D_Luminal_1_1" = celltype3,
                      "D_Luminal_1_2" = celltype4,
                      "D_Luminal_2" = celltype5,
                      "L_Basal" = celltype6,
                      "L_Basal_myoepithelial" = celltype7,
                      "L_Luminal_1_1" = celltype8,
                      "L_Luminal_1_2" = celltype9,
                      "L_Luminal_2" = celltype10)
...
#write.csv(df_diff,file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DE_BRCA_TNBC_vs_ER_1topic_iter4.csv")
write.csv(df_diff,file="~/Swapna/WORK/McGill/projects/GTM_decon/manuscript/figures/Fig8_DE_BRCA_TNBC_vs_ER_1topic_iter4_vals.csv")
write.csv(df,file="~/Swapna/WORK/McGill/projects/GTM_decon/manuscript/figures/Fig8_DE_BRCA_TNBC_vs_ER_1topic_iter4_origvals.csv")

##Get bottom20 different genes in cell-type difference 
datalist = list()
topn_df <- data.frame()
for(i in 2:ncol(df_diff)) {       # for-loop over columns
  subset_df <- df_diff %>% arrange(df_diff[,i]) %>% slice(1:20)
  datalist[[i]]<-subset_df
}
topn_df <- do.call(rbind,datalist)
...
#m <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/results_gtm-decon/TCGA/BRCA/DEgenes_type_sparse_75p/1topic_prior_0.9/trainData_trainData_JCVB0_nmar_K10_iter4_metagene_normalized.csv",header=FALSE)
m <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/results_gtm-decon/TCGA/BRCA/DEgenes_type_sparse_75p/1topic/testData_trainData_JCVB0_nmar_K10_iter9_metagene.csv",header=FALSE)
row.names(m) <- row.names(clinical)
ordered_matrix <- m[order(m[,8]),]
categories <- c("ductal","lobular")
pheatmap(ordered_matrix[,c(1,2)],annotation_row = annot_row, color=colorRampPalette(c("white", "red"))(50),show_rownames = F,annotation_colors = annot_colors,fontsize = 16,fontsize_col = 20,cluster_cols = F, cluster_rows = F, labels_col = categories)
pheatmap(ordered_matrix[,c(1:10)],annotation_row = annot_row, color=colorRampPalette(c("white", "red"))(50),show_rownames = F,annotation_colors = annot_colors,fontsize = 16,fontsize_col = 20,cluster_cols = F, cluster_rows = F, labels_col = categories)
categories <- c("IDC - Basal", "IDC - Basal_Myo", "IDC - Luminal_1_1", "IDC - Luminal_1_2", "IDC - Luminal_2","ILC - Basal", "ILC - Basal_Myo", "ILC - Luminal_1_1", "ILC - Luminal_1_2", "ILC - Luminal_2")
...
write.csv(all_pvals,file="~/Swapna/WORK/McGill/projects/GTM_decon/manuscript/figures/Fig8_supp_all_pvals_n100000.csv")
write.csv(all_pvals_lt0.05,file="~/Swapna/WORK/McGill/projects/GTM_decon/manuscript/figures/Fig8_supp_all_pvals_lt0.05_n100000.csv")

all_pvals_lt0.05 <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/manuscript/figures/Fig8_supp_all_pvals_lt0.05_n100000.csv")
...
deseq2 <- read.csv(file="~/Swapna/WORK/McGill/projects/GTM_decon/scripts/figures/DESEQ2_BRCA_type_lobular_vs_ductal_lfc0.58_padj0.1.csv",header=TRUE)
...
pheatmap(mat_de[-c(1:6)],annotation_row = annot_row, annotation_colors = ann_colors, cluster_cols=F,cluster_rows=T,color=colorRampPalette(c("white","blue"))(50),show_rownames = F,fontsize = 12)
...
pheatmap(mat_de_phi_diff_matrix,annotation_row = annot_row, annotation_colors = ann_colors, cluster_cols=F,cluster_rows=T,color=colorRampPalette(c("red","white","blue"))(50),show_rownames = F,fontsize = 8)
...
## DESEQ2-based heatmap

deseq2_up <- deseq2[deseq2$log2FoldChange>0,]
deseq2_down <- deseq2[deseq2$log2FoldChange<0,]
pheatmap(deseq2_up$log2FoldChange,cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("white","blue"))(50),cellwidth = 20,show_rownames = F)
pheatmap(deseq2_down$log2FoldChange,cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("red","white"))(50),cellwidth = 20,show_rownames = F)
#padj
pheatmap(deseq2$padj,cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("darkgrey","white"))(50),cellwidth = 20,show_rownames = F,fontsize = 12)

df_diff_deseq_up <- df_diff[match(deseq2_up$Genes,df_diff$V1),]
df_diff_deseq_down <- df_diff[match(deseq2_down$Genes,df_diff$V1),]
pheatmap(df_diff_deseq_up[-c(1:6)],cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("white","darkblue"))(50),show_rownames = F,fontsize = 12)
pheatmap(df_diff_deseq_down[-c(1:6)],cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("red","white"))(50),show_rownames = F,fontsize = 12)
...
genes_of_interest <- deseq2_up$Genes
...
genes_of_interest <- deseq2_down$Genes
background_genes <- df$V1
ORA_DE <- enricher(genes_of_interest, pvalueCutoff = 0.05, pAdjustMethod = "BH", universe = background_genes, TERM2GENE = select(h_gene_sets,gs_name,gene_symbol))

#Heatmap based on overall absolute differences
abs_rowsums <- rowSums(abs(df_diff[2:ncol(df_diff)]))
ndf<-cbind(genes,abs_rowsums)
top_diff_genes <- ndf %>% arrange(desc(ndf[,2])) %>% slice(1:1000)
deseq2_annot <- top_diff_genes$V1 %in% deseq2$Genes
length(deseq2_annot[deseq2_annot == TRUE])

annot_row = data.frame(DESEQ2 = factor(deseq2_annot))
ann_colors = list(DESEQ2=c("TRUE" = "red", "FALSE" = "white"))
mat_de_phi_diff <- df_diff[df_diff$V1 %in% top_diff_genes$V1,]
rownames(annot_row) <- rownames(mat_de_phi_diff)
colnames(mat_de_phi_diff)<-c("Genes","Basal","Basal_myoepithelial", "Luminal_1_1", "Luminal_1_2", "Luminal_2","Basal","Basal_myoepithelial", "Luminal_1_1", "Luminal_1_2", "Luminal_2")
pheatmap(mat_de_phi_diff[-c(1:6)],annotation_row = annot_row, annotation_colors = ann_colors, cluster_cols=T,cluster_rows=T,color=colorRampPalette(c("red","white","blue"))(50),show_rownames = F,fontsize = 12)

## Enrichment analysis based on top1000 DE genes
library(msigdbr)
...
#pheatmap(as.matrix(all_pvals_lt0.05[-c(1)]),annotation_row = annot_row, annotation_colors = ann_colors, cluster_cols=F,cluster_rows=T,color=colorRampPalette(c("white","blue"))(50),show_rownames = F,fontsize = 12)




## GSEA not working with differences; lets try log2FC

#Holds differences between the phenotypes for each celltype
celltype1 <- log2(df$V3/df$V8)
celltype2 <- log2(df$V4/df$V9)
celltype3 <- log2(df$V5/df$V10)
celltype4 <- log2(df$V6/df$V11)
celltype5 <- log2(df$V7/df$V12)

celltype6 <- log2(df$V8/df$V3)
celltype7 <- log2(df$V9/df$V4)
celltype8 <- log2(df$V10/df$V5)
celltype9 <- log2(df$V11/df$V6)
celltype10 <- log2(df$V12/df$V7)


df_log2fc <- data.frame("Genes" = genes,
                        "T_Basal" = celltype1,
                        "T_Basal_myoepithelial" = celltype2,
                        "T_Luminal_1_1" = celltype3,
                        "T_Luminal_1_2" = celltype4,
                        "T_Luminal_2" = celltype5,
                        "E_Basal" = celltype6,
...
gseaDat <- df_log2fc
ranks <- gseaDat$E_Basal
names(ranks) <- rownames(gseaDat)
fgseaRes <- fgsea(h_gene_sets, ranks, minSize = 15, maxSize = 500, eps = 0.0)
head(fgseaRes[order(pval), ],5)

ranks <- gseaDat$E_Basal_myoepithelial
names(ranks) <- rownames(gseaDat)
fgseaRes <- fgsea(h_gene_sets, ranks, minSize = 15, maxSize = 500, eps = 0.0, scoreType = "pos")
head(fgseaRes[order(pval), ],5)

ranks <- gseaDat$E_Luminal_1_1
names(ranks) <- rownames(gseaDat)
fgseaRes <- fgsea(h_gene_sets, ranks, minSize = 15, maxSize = 500, eps = 0.0, scoreType = "pos")
head(fgseaRes[order(pval), ],5)

ranks <- gseaDat$E_Luminal_1_2
names(ranks) <- rownames(gseaDat)
fgseaRes <- fgsea(h_gene_sets, ranks, minSize = 15, maxSize = 500, eps = 0.0, scoreType = "pos")
head(fgseaRes[order(pval), ],5)

ranks <- gseaDat$E_Luminal_2
names(ranks) <- rownames(gseaDat)
fgseaRes <- fgsea(h_gene_sets, ranks, minSize = 15, maxSize = 500, eps = 0.0, scoreType = "pos")
head(fgseaRes[order(pval), ],5)
...
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_TNBC_vs_ER_Up_log2FC.csv",header=TRUE)
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_TNBC_vs_ER_Up_padj.csv",header=TRUE)
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_TNBC_vs_ER_Down.csv",header=TRUE)
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_ER_vs_TNBC_together.csv",header=TRUE)
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_ER_vs_TNBC_together_Up.csv",header=TRUE)
#f <- read.csv(file="~/WORK/McGill/projects/GTM_decon/scripts/figures/DEgenes_BRCA_ER_vs_TNBC_together_Down.csv",header=TRUE)
...
#padj
pheatmap(f[3],cluster_rows = F, cluster_cols = F,color=colorRampPalette(c("darkgrey","white"))(50),cellwidth = 20,show_rownames = F,fontsize = 12)
#differences in phi
...
pheatmap(as.matrix(f[19:23]),cluster_rows = F, cluster_cols = T,color=colorRampPalette(c("red","white","blue"))(50),show_rownames=F,cellwidth=20,fontsize = 8)
