		...
		while (getline(cell_ohe_file,line))
		{
			...
			*/
			...
			if (num_topics == 1){
				...
				//double highest_prob = 0.6;
				//double highest_prob = 0.8;
				double highest_prob = 0.9;
				//double highest_prob = 0.7;
				//double highest_prob = 0.95;
				//double highest_prob = 0.18;
				//double highest_prob = 0.1;
				for(int i = 0; i < tokens.size(); i++)
					if(stoi(tokens[i]) > 0)
					{
						//highest_prob = ((double)rand()/RAND_MAX)* (0.91-0.89) + 0.89;
						ofs_prior_file << cell_id << ' ' << i << ' ' << highest_prob << '\n';
					}
					else
					{
						...
						//
						//double eqNum = (0.1)/(tokens.size() - 1);
						//randNum = (0.5*eqNum) + ((double)rand()/RAND_MAX + 1) * (eqNum - 0.7*eqNum);
						//
						//double eqNum = (0.2)/(tokens.size() - 1);
						//randNum = (0.5*eqNum) + ((double)rand()/RAND_MAX + 1) * (eqNum - 0.7*eqNum);
						//double eqNum = (0.3)/(tokens.size() - 1);
						//randNum = (0.5*eqNum) + ((double)rand()/RAND_MAX + 1) * (eqNum - 0.7*eqNum);
						//double eqNum = (0.05)/(tokens.size() - 1);
						//randNum = (0.5*eqNum) + ((double)rand()/RAND_MAX + 1) * (eqNum - 0.7*eqNum);
						ofs_prior_file << cell_id << ' ' << i << ' ' << randNum << '\n';
					}
			}
				       ...
				       else
				       {
					       ...
					       randNum = ((double)rand()/RAND_MAX)* (0.01-0.001) + 0.001;
					       //randNum = ((double)rand()/RAND_MAX)* (0.0001-0.00001) + 0.00001;
					       //randNum = ((double)rand()/RAND_MAX)* (0.01-0.001) + 0.001;
				               ofs_prior_file << cell_id << ' ' << j << ' ' << randNum << '\n';
					       //randNum = ((double)rand()/RAND_MAX)* (0.01-0.001) + 0.001;
					       ofs_prior_file << cell_id << ' ' << j+1 << ' ' << randNum << '\n';
